...
public class Game {
    ...
    public Game interact(String entityId) throws IllegalArgumentException, InvalidActionException {
        PlaceableEntity e = map.getEntity(entityId);
    ...
    public void registerOnTick(Runnable r, int priority, String id) {
        sub.add(new ComparableCallback(r, priority, id));
    }
    ...
    public int tick() {
        ...
        tickCount++;
        System.out.println();
        System.out.println(map.getEntities().stream().filter(entity -> !(entity instanceof Wall))
                .map(entity -> (entity.getId().substring(0, 5) + " "
                        + entity.getClass().toString().substring(entity.getClass().toString().length() - 10)))
    ...
    public int getDefeatedEnemyCount() {
        return player.getDefeatedEnemyCount();
    }
